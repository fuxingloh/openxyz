...
// for groups, e.g. `-1001234567890`). Empty = DM-only; otherwise the janitor
// joins conversations in these groups and replies without needing a mention.
const groupAllowlist = env.TELEGRAM_GROUP_ALLOWLIST.describe(
  "Comma-separated Telegram group/chat IDs where this janitor may participate (empty = DM-only)",
)
  .default("")
  .transform(
    (s) =>
 ...
 * backfill. Group `👀` ack on reply is auto-fired by the runtime.
 * `message.raw.chat.id` is the bare Telegram chat ID; `thread.channel.id`
 * is chat-sdk's `telegram:<chatId>` form.
 */
