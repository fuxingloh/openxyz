...
export class LibSqlStateAdapter implements StateAdapter {
  ...
  async extendLock(lock: Lock, ttlMs: number): Promise<boolean> {
    ...
    const row = stmt.get(now + ttlMs, now, this.keyPrefix, lock.threadId, lock.token, now);
    return row !== undefined;
  }
  ...
  async dequeue(threadId: string): Promise<QueueEntry | null> {
    ...
    const del = await this.db.prepare("DELETE FROM chat_state_queues WHERE seq = ?");
  ...
  private ensureConnected(): void {
    if (!this.connected) {
      throw new Error("LibSqlStateAdapter is not connected. Call connect() first.");
    }
  }
}
...
export function createLibSqlState(options: CreateLibSqlStateOptions = {}): LibSqlStateAdapter {
  ...
  if (!url) {
    throw new Error("libSQL url is required. Set TURSO_DATABASE_URL or provide it in options.");
  }
